...
import { deleteLog, deleteNote, insertLog, insertNote, loadLogs, loadModuleMeta, loadNotes, updateNote, upsertModuleMeta } from '../api';
...
export default function ProjectDetailView() {
  ...
  const subtitleMap = useMemo(() => Object.fromEntries(moduleMeta.map((m) => [m.module_id, m.subtitle])), [moduleMeta]);

  useEffect(() => {
    Promise.all([loadLogs(projectId), loadNotes(projectId), loadModuleMeta(projectId)])
      .then(([logData, noteData, metaData]) => {
        setLogs(logData);
        setNotes(noteData);
        setModuleMeta(metaData);
      })
  ...
  const saveSubtitle = async (moduleId, subtitle) => {
    if (!requireOwner()) return;
    ...
    if (existing) {
      ...
      setModuleMeta((prev) => [...prev, { project_id: projectId, module_id: moduleId, subtitle: trimmed }]);
    }
    try {
      await upsertModuleMeta({ project_id: projectId, module_id: moduleId, subtitle: trimmed });
      showToast('已保存');
    } catch {
      setModuleMeta(old);
      showToast('保存失败');
  ...
  return (
    <main className="mx-auto max-w-md px-5 pb-32">
      <div className="card p-4">
        <div className="font-bold text-xl">{projectId}</div>
        <div className="mono mt-1 text-xs" style={{ color: 'var(--ink-faint)' }}>
          {logs.length} 次打卡 · {unresolvedCount} 个待解决难点
        </div>
      </div>
      {tab === 'plan' && <PlanView modules={plan} subtitleMap={subtitleMap} doneItemIds={doneItemIds} todayDoneIds={todayDoneIds} notes={notes.filter((note) => note.type === 'note')} onCheckin={setCheckInItem} onAddNote={(item) => setAddNoteFor({ type: 'note', item })} onSaveSubtitle={saveSubtitle} />}
