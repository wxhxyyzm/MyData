...
import { Flame, MapPin, Pencil, Trash2 } from 'lucide-react';
import { MOOD_OPTIONS } from '../presets';
...
export default function LogItem({ entry, onDelete, onEdit }) {
  ...
  const mood = MOOD_OPTIONS.find((item) => item.value === moodValue);
  const difficulty = entry.difficulty || entry.extras?.difficulty || 0;
  const location = entry.location || entry.extras?.location || entry.extras?.customLocation;
      ...
      <div className="flex items-center justify-between gap-4">
        <button className="flex-1 text-left" onClick={() => setExpanded((value) => !value)}>
          ...
          <div className="mono mt-1 text-xs" style={{ color: 'var(--ink-soft)' }}>
            ...
            {location && <> · <MapPin size={10} style={{ display: 'inline', verticalAlign: '-1px' }} /> {location}</>}
          </div>
        </button>
